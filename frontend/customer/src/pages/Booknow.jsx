...
import { confirmBooking, getEstimate } from '../controllers/bookings';
...
export default class Booknow extends Component {
    ...
    handleEstimate = e => {
        e.preventDefault();
        var start = ((this.state.startDate.getMonth() > 8) ? (this.state.startDate.getMonth() + 1) : ('0' + (this.state.startDate.getMonth() + 1))) + '/' + ((this.state.startDate.getDate() > 9) ? this.state.startDate.getDate() : ('0' + this.state.startDate.getDate())) + '/' + this.state.startDate.getFullYear()
        var end = ((this.state.endDate.getMonth() > 8) ? (this.state.endDate.getMonth() + 1) : ('0' + (this.state.endDate.getMonth() + 1))) + '/' + ((this.state.endDate.getDate() > 9) ? this.state.endDate.getDate() : ('0' + this.state.endDate.getDate())) + '/' + this.state.endDate.getFullYear()
        var total_guests = this.state.totalGuests;
        getEstimate(start, end, total_guests, this.state.roomDetails.id, parseInt(sessionStorage.getItem("userId"))).then(res => {
            this.setState({estimate: res.data.data.finalPrice})
            localStorage.setItem("finalEstimate", res.data.data.finalPrice)
            console.log(res.data.data.finalPrice);
        }).catch(err => {
            console.log(err);
        })
    }
            ...
            <form onSubmit={this.handleSubmit}>
                <div className="row">
                    <div className="col-md-10 mx-auto col-12 card shadow-lg border-0 p-4">
                        <div>
                            <h1 className="display-4">Booking - {name}</h1>
                        </div>
                        <div className="row">
                        ...
                        <div className="row">
                            ...
                            <div className="col-md-6 col-12">
                                <h6 className="font-weight-bold">Price per day : ${base_price}</h6>
                                <h6 className="font-weight-bold">Total Price to be paid : ${this.state.estimate}</h6>
                                <button className="btn btn-block btn-outline-primary" onClick={this.handleEstimate}>Estimate Booking</button>
                            </div>
                        </div>
